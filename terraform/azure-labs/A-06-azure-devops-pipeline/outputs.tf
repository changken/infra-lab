output "devops_project_url" {
  ...
  value = "${var.azuredevops_org_url}/${azuredevops_project.project.name}"
}
...
output "pipeline_url" {
  ...
  value = "${var.azuredevops_org_url}/${azuredevops_project.project.name}/_build?definitionId=${azuredevops_build_definition.pipeline.id}"
}
