...
├── user-data-aws.sh             # AWS cloud-init（Tailscale + K3s）
...
| AWS t3.medium | ~$0.0416/hr (~$30/月) |
| **合計（長時間開機）** | **~$60/月** ⚠️ |

> **注意**：此費用遠超 `terraform/CLAUDE.md` 的 $48/月 預算上限。
> 實際使用時應透過 `az vm deallocate` / `aws ec2 stop-instances` 在不需要時關機。
...
| `aws_instance_type` | ❌ | 預設 `t3.medium` |
